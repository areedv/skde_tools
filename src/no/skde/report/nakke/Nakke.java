/**
 * no.skde.report.nakke
 * Nakke.java March 18 2016 Are Edvardsen
 * 
 * 
 *  Copyleft 2016 SKDE
 */

package no.skde.report.nakke;
...
public class Nakke extends JRDefaultScriptlet {
	...
	private void generateReport() {
		try {
			log.info("Start generating R report using " + Nakke.class.getName());
			
			//TODO
			// Make log entry if class is built as a snapshot. SET MANUALLY!
			boolean classIsSnapshot = true;
			if (classIsSnapshot) {
				log.warn(Nakke.class.getName() + " is a snapshot. Not to be used in production environment");
			}
			...
			try {
				...
				if (beginDate == null) {
					beginDate = new SimpleDateFormat("yyyy-MM-dd").parse("2013-01-01");
				}
			...
			String fileBaseName = "Nakke_" + reportName + "-";
